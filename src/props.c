...
static void endElement(void *userData, const XML_Char *name) {
    ...
    if (strcmp(name, "status") == 0) {
        ...
        size_t path_len;
        log_print_old(LOG_INFO, "href: %s", state->estate.current_data);
    ...
    // @TODO: Update Valhalla server to use HTTP/1.1 410 Gone instead.
        ...
        log_print_old(LOG_DEBUG, "Response for path: %s (code %lu, size, %lu)", state->rstate.path, state->rstate.status_code, state->rstate.st.st_size);
...
static size_t write_parsing_callback(void *contents, size_t length, size_t nmemb, void *userp) {
    ...
    log_print_old(LOG_DEBUG, "Got chunk of %u bytes.", real_size);
    ...
    if (!state->failure) {
        if (XML_Parse(parser, contents, real_size, 0) == 0) {
            ...
            log_print_old(LOG_NOTICE, "Parsing response buffer of length %u failed with error: %s", real_size, XML_ErrorString(error_code));
            state->failure = true;
        }
    }
...
int simple_propfind(const char *path, size_t depth, props_result_callback results, void *userdata) {
    ...
    log_print_old(LOG_INFO, "About to perform PROPFIND.");
    res = curl_easy_perform(session);

    if (res != CURLE_OK) {
        log_print_old(LOG_WARNING, "PROPFIND failed: %s", curl_easy_strerror(res));
        goto finish;
    }
    ...
    if (response_code == 207) {
        // Finalize parsing.
        if (state.failure) {
            log_print_old(LOG_WARNING, "Could not finalize parsing of the 207 response because it's already in a failed state.");
            goto finish;
        }
        else if (XML_Parse(parser, NULL, 0, 1) == 0) {
            int error_code = XML_GetErrorCode(parser);
            log_print_old(LOG_WARNING, "Finalizing parsing failed with error: %s", XML_ErrorString(error_code));
        }
    }
        ...
        log_print_old(LOG_WARNING, "PROPFIND failed with response code: %u", response_code);
        goto finish;
    }

    log_print_old(LOG_DEBUG, "PROPFIND completed on path %s", path);
    ret = 0;
