...
static void get_fresh_fd(filecache_t *cache,
    ...
    do {
        ...
        if (code == 304) {
            ...
            // to avoid any leak.
            if (sdata->fd > 0) {
                log_print(LOG_NOTICE, SECTION_FILECACHE_OPEN, "get_fresh_fd: Closing existing fd %d for path %s because it is being replaced by fd %d.", sdata->fd, path, response_fd);
                close(sdata->fd);
            }
