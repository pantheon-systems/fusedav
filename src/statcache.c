...
void stat_cache_print_stats(void) {
    log_print_old(LOG_NOTICE, "Stat Cache Operations:");
    log_print_old(LOG_NOTICE, "  local_gen:   %u", FETCH(local_gen));
    ...
    log_print_old(LOG_NOTICE, "  value_get:   %u", FETCH(value_get));
    log_print_old(LOG_NOTICE, "  updated_ch:  %u", FETCH(updated_ch));
    log_print_old(LOG_NOTICE, "  read_updated:%u", FETCH(read_updated));
    log_print_old(LOG_NOTICE, "  value_set:   %u", FETCH(value_set));
    log_print_old(LOG_NOTICE, "  delete:      %u", FETCH(delete));
    log_print_old(LOG_NOTICE, "  del_parent:  %u", FETCH(del_parent));
    log_print_old(LOG_NOTICE, "  iter_free:   %u", FETCH(iter_free));
    log_print_old(LOG_NOTICE, "  iter_init:   %u", FETCH(iter_init));
    log_print_old(LOG_NOTICE, "  iter_current:%u", FETCH(iter_current));
    log_print_old(LOG_NOTICE, "  iter_next:   %u", FETCH(iter_next));
    log_print_old(LOG_NOTICE, "  enumerate:   %u", FETCH(enumerate));
    log_print_old(LOG_NOTICE, "  has_child:   %u", FETCH(has_child));
    log_print_old(LOG_NOTICE, "  delete_older:%u", FETCH(delete_older));
    log_print_old(LOG_NOTICE, "  prune:       %u", FETCH(prune));
}
...
unsigned long stat_cache_get_local_generation(void) {
    ...
    if (counter == 0) {
        ...
        counter = time(NULL);
        //log_print_old(LOG_DEBUG, "Pre-shift counter: %lu", counter);
        counter <<= 24;
        //log_print_old(LOG_DEBUG, "Initialized counter: %lu", counter);
    }
    ret = ++counter;
    pthread_mutex_unlock(&counter_mutex);
    //log_print_old(LOG_DEBUG, "stat_cache_get_local_generation: %lu", ret);
    return ret;
}

int print_stat(struct stat *stbuf, const char *title) {
    if (debug) {
        log_print_old(LOG_DEBUG, "stat: %s", title);
        log_print_old(LOG_DEBUG, "  .st_mode=%04o", stbuf->st_mode);
        log_print_old(LOG_DEBUG, "  .st_nlink=%ld", stbuf->st_nlink);
        log_print_old(LOG_DEBUG, "  .st_uid=%d", stbuf->st_uid);
        log_print_old(LOG_DEBUG, "  .st_gid=%d", stbuf->st_gid);
        log_print_old(LOG_DEBUG, "  .st_size=%ld", stbuf->st_size);
        log_print_old(LOG_DEBUG, "  .st_blksize=%ld", stbuf->st_blksize);
        log_print_old(LOG_DEBUG, "  .st_blocks=%ld", stbuf->st_blocks);
        log_print_old(LOG_DEBUG, "  .st_atime=%ld", stbuf->st_atime);
        log_print_old(LOG_DEBUG, "  .st_mtime=%ld", stbuf->st_mtime);
        log_print_old(LOG_DEBUG, "  .st_ctime=%ld", stbuf->st_ctime);
    }
    return E_SC_SUCCESS;
}
...
static char *path2key(const char *path, bool prefix) {
    ...
    log_print_old(LOG_DEBUG, "path2key(%s, %i)", path, prefix);
...
void stat_cache_close(stat_cache_t *cache, struct stat_cache_supplemental supplemental) {
    ...
    if (supplemental.options != NULL) {
        ...
        log_print_old(LOG_DEBUG, "leveldb_options_destroy");
    }
...
struct stat_cache_value *stat_cache_value_get(stat_cache_t *cache, const char *path, bool skip_freshness_check, GError **gerr) {
    ...
    log_print_old(LOG_DEBUG, "CGET: %s", key);
    ...
    if (value == NULL) {
        log_print_old(LOG_DEBUG, "stat_cache_value_get miss on path: %s", path);
        return NULL;
    }

    // @TODO this should be a gerror
    if (vallen != sizeof(struct stat_cache_value)) {
        log_print_old(LOG_NOTICE, "Length %lu is not expected length %lu.", vallen, sizeof(struct stat_cache_value));
    }
    ...
    if (!skip_freshness_check) {
        ...
        //log_print_old(LOG_DEBUG, "Current time: %lu", current_time);
        if (current_time - value->updated > CACHE_TIMEOUT) {
            char *directory;
            time_t directory_updated;
            int is_dir;

            log_print_old(LOG_DEBUG, "Stat entry %s is %lu seconds old.", path, current_time - value->updated);
            ...
            //log_print_old(LOG_DEBUG, "Directory contents for %s are %lu seconds old.", directory, (current_time - directory_updated));
            free(directory);
            if (current_time - directory_updated > CACHE_TIMEOUT) {
                log_print_old(LOG_DEBUG, "%s is too old.", path);
                free(value);
                return NULL;
            }
        }
...
time_t stat_cache_read_updated_children(stat_cache_t *cache, const char *path, GError **gerr) {
    ...
    log_print_old(LOG_DEBUG, "Children for directory %s were updated at timestamp %lu.", path, ret);
...
void stat_cache_value_set(stat_cache_t *cache, const char *path, struct stat_cache_value *value, GError **gerr) {
    ...
    if (path == NULL) {
        log_print_old(LOG_NOTICE, "stat_cache_value_set: input path is null");
        return;
    }
    ...
    key = path2key(path, false);
    log_print_old(LOG_DEBUG, "CSET: %s (mode %04o)", key, value->st.st_mode);
...
void stat_cache_delete(stat_cache_t *cache, const char *path, GError **gerr) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_delete: %s", key);
    ...
    log_print_old(LOG_DEBUG, "stat_cache_delete: exit %s", path);
...
void stat_cache_delete_parent(stat_cache_t *cache, const char *path, GError **gerr) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_delete_parent: %s", path);
    if ((p = path_parent(path))) {
        int l = strlen(p);

        log_print_old(LOG_DEBUG, "stat_cache_delete_parent: deleting parent %s", p);
        if (strcmp(p, "/") && l) {
            if (p[l-1] == '/')
                p[l-1] = 0;
        }
        ...
        log_print_old(LOG_DEBUG, "stat_cache_delete_parent: not deleting parent, deleting child %s", path);
...
static struct stat_cache_iterator *stat_cache_iter_init(stat_cache_t *cache, const char *path_prefix) {
    ...
    log_print_old(LOG_DEBUG, "creating leveldb iterator for prefix %s", iter->key_prefix);
...
static struct stat_cache_entry *stat_cache_iter_current(struct stat_cache_iterator *iter) {
    ...
    log_print_old(LOG_DEBUG, "fetched key: %s", key);
    ...
    if (strncmp(key, iter->key_prefix, iter->key_prefix_len - 1) != 0) {
        log_print_old(LOG_DEBUG, "Key %s does not match prefix %s for %lu characters. Ending iteration.", key, iter->key_prefix, iter->key_prefix_len);
        return NULL;
    }
    ...
    entry->key = key;
    entry->value = value;
    log_print_old(LOG_DEBUG, "iter_current: key = %s; value = %s", key, value);
    return entry;
}
...
static void stat_cache_list_all(stat_cache_t *cache, const char *path) {
    ...
    while (leveldb_iter_valid(iter)) {
        //log_print_old(LOG_DEBUG, "Listing key: %s", leveldb_iter_key(iter, &klen));
        ...
        if (S_ISDIR(itervalue->st.st_mode)) {
            ...
            log_print_old(LOG_DEBUG, "Listing directory: %s", iterkey);

            value = stat_cache_value_get(cache, key2path(iterkey));
            if (value) {
                log_print_old(LOG_DEBUG, "Direct get mode: %04o", value->st.st_mode);
                free(value);
            }
        }
...
int stat_cache_enumerate(stat_cache_t *cache, const char *path_prefix, void (*f) (const char *path, const char *child_path, void *user), void *user, bool force) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_enumerate(%s)", path_prefix);
    ...
    if (!force) {
        ...
        if (current_time - timestamp > CACHE_TIMEOUT) {
            log_print_old(LOG_DEBUG, "cache value too old: %s %u", path_prefix, (unsigned)timestamp);
            return -STAT_CACHE_OLD_DATA;
        }
    }

    iter = stat_cache_iter_init(cache, path_prefix);
    log_print_old(LOG_DEBUG, "iterator initialized with prefix: %s", iter->key_prefix);

    while ((entry = stat_cache_iter_current(iter))) {
        log_print_old(LOG_DEBUG, "key: %s", entry->key);
        log_print_old(LOG_DEBUG, "fn: %s", entry->key + (iter->key_prefix_len - 1));
    ...
    log_print_old(LOG_DEBUG, "Done iterating: %u items.", found_entries);
...
bool stat_cache_dir_has_child(stat_cache_t *cache, const char *path) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_dir_has_children(%s)", path);

    iter = stat_cache_iter_init(cache, path);
    if ((entry = stat_cache_iter_current(iter))) {
        has_children = true;
        log_print_old(LOG_DEBUG, "stat_cache_dir_has_children(%s); entry \'%s\'", path, entry->key);
        free(entry);
    }
    stat_cache_iterator_free(iter);
...
void stat_cache_delete_older(stat_cache_t *cache, const char *path_prefix, unsigned long minimum_local_generation, GError **gerr) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_delete_older: %s", path_prefix);
    iter = stat_cache_iter_init(cache, path_prefix);
    while ((entry = stat_cache_iter_current(iter))) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_delete_older: calling stat_cache_prune on %s", path_prefix);
...
void stat_cache_prune(stat_cache_t *cache) {
    ...
    log_print_old(LOG_DEBUG, "stat_cache_prune: enter");

    boptions = bloomfilter_init(0, NULL, 0, &errptr);
    if (boptions == NULL) {
        log_print_old(LOG_WARNING, "stat_cache_prune: failed to allocate bloom filter: %s", errptr);
        free(errptr);
        return;
    }

    // We need to make sure the base_directory is in the filter before continuing
    log_print_old(LOG_DEBUG, "stat_cache_prune: attempting base_directory %s)", base_directory);
    if (bloomfilter_add(boptions, base_directory, strlen(base_directory)) < 0) {
        log_print_old(LOG_WARNING, "stat_cache_prune: seed: error on ITERKEY: \'%s\')", path);
        return;
    }

    log_print_old(LOG_DEBUG, "stat_cache_prune: put base_directory %s in filter", base_directory);
    ...
    while (pass < passes) {
        ...
        log_print_old(LOG_DEBUG, "stat_cache_prune: Changing pass:%d (%d)", pass, passes);
        ...
        for (; leveldb_iter_valid(iter); leveldb_iter_next(iter)) {
            ...
            if (key == NULL) {
                log_print_old(LOG_NOTICE, "stat_cache_prune: ignoring malformed iterkey");
            ...
            log_print_old(LOG_DEBUG, "stat_cache_prune: ITERKEY: \'%s\' :: %s :: %s", iterkey, path, key);
            itervalue = (const struct stat_cache_value *) leveldb_iter_value(iter, &vlen);
            ...
            if (depth == 0 /*&& errno != 0*/) {
                log_print_old(LOG_DEBUG, "stat_cache_prune: depth = 0; break:%d, %d", depth, errno);
                break;
            }
            ...
            if (depth > max_depth) {
                ...
                log_print_old(LOG_DEBUG, "stat_cache_prune: New max_depth %d (%d :: %d %d)", max_depth, depth, pass, passes);
            }

            if ((pass == 0 && depth <= 9) || (pass == 1 && (depth >= 10 && depth <= 99)) ||
                (pass == 2 && (depth >= 100 && depth <= 999)) || (pass == 3 && depth >= 1000)) {

                log_print_old(LOG_DEBUG, "stat_cache_prune: Pass %d (%d)", pass, passes);
                ++visited_entries;
                ...
                if (slash == NULL) {
                    log_print_old(LOG_INFO, "stat_cache_prune: ignoring errant entry \'%s\'", path);
                    continue;
                }
                ...
                if (bloomfilter_exists(boptions, path, strlen(path))) {
                    log_print_old(LOG_DEBUG, "stat_cache_prune: exists in bloom filter\'%s\'", path);
                    ...
                    if (S_ISDIR(itervalue->st.st_mode)) {
                        ...
                        log_print_old(LOG_DEBUG, "stat_cache_prune: add path to filter \'%s\')", path);
                        if (bloomfilter_add(boptions, path, strlen(path)) < 0) {
                            log_print_old(LOG_ERR, "stat_cache_prune: error on bloomfilter_add: \'%s\')", path);
                            break;
                        }
                    }
                }
                else {
                    log_print_old(LOG_DEBUG, "stat_cache_prune: doesn't exist in bloom filter \'%s\'", path);
                    ++deleted_entries;
                    // Reset to original, complete path
                    if (slash) slash[0] = '/';
                    log_print_old(LOG_DEBUG, "stat_cache_prune: deleting \'%s\'", path);
                    stat_cache_delete(cache, path, NULL);
                }
            }
        }
        ++pass;
        log_print_old(LOG_DEBUG, "stat_cache_prune: updating pass %d", pass);
    }
    ...
    while (leveldb_iter_valid(iter)) {
        ...
        if (basepath == NULL) {
            log_print_old(LOG_NOTICE, "stat_cache_prune: key error in updated_children entry: %s", iterkey);
            woptions = leveldb_writeoptions_create();
            leveldb_delete(cache, woptions, iterkey, strlen(iterkey) + 1, &errptr);
            leveldb_writeoptions_destroy(woptions);
            if (errptr != NULL) {
                log_print_old(LOG_ERR, "stat_cache_prune: leveldb_delete error: %s", errptr);
                free(errptr);
            }
            break;
        }
        ...
        if (bloomfilter_exists(boptions, basepath, strlen(basepath))) {
            log_print_old(LOG_DEBUG, "stat_cache_prune: exists in bloom filter (basepath of)\'%s\'", iterkey);
        }
        else {
            log_print_old(LOG_DEBUG, "stat_cache_prune: updated_children: deleting \'%s\'", iterkey);
            ++deleted_entries;
    ...
    log_print_old(LOG_NOTICE, "stat_cache_prune: visited %d cache entries; deleted %d; had %d issues; elapsedtime %lu", visited_entries, deleted_entries, issues, elapsedtime);
