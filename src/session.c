...
CURL *session_request_init(const char *path, const char *query_string, bool tmp_session, rwp_t rwp) {
    CURL *session;
    char *full_url = NULL;
    char *escaped_path;
    ...
    if (use_saint_mode()) {
        ...
        if (rwp == WRITE) stats_counter("saint_write", 1, 1.0);
        else if (rwp == READ) stats_counter("saint_read", 1, 1.0);
        else if (rwp == PROPFIND) stats_counter("saint_propfind", 1, 1.0);
        return NULL;
    }
