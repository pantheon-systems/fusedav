...
// We are using a byte not a bit to store a value
int bloom_add(const char *key, int klen, unsigned long adler, char *bloombits) {
    int idx;
    unsigned short hashvalue;

    log_print(LOG_DEBUG, "bloom_add: enter \'%s\' :: a %ul", key, adler);
    adler = adler32(adler, (const Bytef *)key, klen);
    for (idx = 0; idx < 2; idx++) {
        hashvalue = adler & 0xffff;
        log_print(LOG_DEBUG, "bloom_add: \'%s\' :: v %ul :: a %ul :: kl %d", key, hashvalue, adler, klen);
...
bool bloom_exists(const char *key, int klen, unsigned long adler, char *bloombits) {
    ...
    log_print(LOG_DEBUG, "bloom_exists: enter \'%s\' :: a %ul", key, adler);
    adler = adler32(adler, (const Bytef *)key, klen);
    for (idx = 0; idx < 2; idx++) {
        hashvalue = adler & 0xffff;
        log_print(LOG_DEBUG, "bloom_exists: \'%s\' :: v %ul :: %ul :: kl %d", key, hashvalue, adler, klen);
